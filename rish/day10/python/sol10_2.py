...
def draw_crt(cycle_number: int):
    # based on the position of the sprite and the current cycle number
    # decides on character to add to crt row (. or a #)
    # then determines if the character should be added to same crt row or 
    # added to a new row

    global crt_rows, register_x, current_cycle

    # calculate sprite position
    sprite_position = [ register_x - 1, register_x, register_x + 1] 
    print(f"Sprite position is {sprite_position}")

    # derive row_cycle from cycle number
    row_cycle = cycle_number % 40

    # decide on the character to print
    if (row_cycle -1 ) in sprite_position:
        current_char = "#"
    else:
        current_char = "."

    # decide to append to existing row or to a new row
    if ((cycle_number - 1) % 40 == 0):
        # append as a new row
        crt_rows.append(current_char)
    else:
        # append to current row
        crt_rows[-1] = f"{crt_rows[-1]}{current_char}"

    print(f"Current CRT row: {crt_rows[-1]}")
...
def schedule_command(command: str) -> None:
    ...
    global register_x
    print(f"Begin executing command: {command}")
    match command:
